...
using System.IO;
using static DynamicLightsLevelData;
using System.Linq;

[CustomEditor(typeof(DynamicLightsScenarioData))]
public class DynamicLightsScenarioDataEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        /* DynamicLightsScenarioData scenarioData = (DynamicLightsScenarioData)target;
        ...
        if (GUILayout.Button("Generate lighting scenario data"))
        {
            ...
            Debug.Log("Dynamic Lights - Start baking");
        ...
        {
            LoadLightingScenarioScenes(scenarioData);
            GameObject.FindObjectOfType<DynamicLightsLevelData>().LoadLightingScenarioData(scenarioData);
        } 
    ...
    public void LoadLightingScenarioScenes(DynamicLightsScenarioData scenarioData)
    {
        if (UnityEditor.Lightmapping.giWorkflowMode != UnityEditor.Lightmapping.GIWorkflowMode.OnDemand)
        {
            Debug.LogError("Dynamic Lights requires Auto Generate lighting mode disabled.");
            return;
        }

        Debug.Log("Loading scenario " + scenarioData.lightingSceneName);
    ...
    private IEnumerator BuildLightingAsync(DynamicLightsScenarioData scenarioData)
    {
        Scene lightingScene = SceneManager.GetSceneByName(scenarioData.lightingSceneName);
    ...
    public void StoreLightingData()
    {
        ...
        {
            var probes = ScriptableObject.CreateInstance<DynamicLightsProbesAsset>();
    ...
    static void GenerateLightingData(DynamicLightsScenarioData data)
    {
        var newRendererInfos = new List<DynamicLightsLevelData.RendererInfo>();
        ...
        if (Application.isEditor)
            Debug.Log("Dynamic Lights - " + "Stored info for " + gameObjects.ToList().Count + " GameObjects.");
        /*
        ...
        {
            DynamicLightsLevelData.RendererInfo terrainRendererInfo = new DynamicLightsLevelData.RendererInfo();
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            {
                DynamicLightsLevelData.RendererInfo info = new DynamicLightsLevelData.RendererInfo();
                info.transformHash = DynamicLightsLevelData.GetStableHash(renderer.gameObject.transform);
